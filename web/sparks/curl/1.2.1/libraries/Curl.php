...
class Curl {
	...
	public function upload($params = array(), $options = array())
	{
		// Add in the specific options provided
		$this->options($options);
...
// Set two default options, and merge any extra ones in
		...
		if ( ! ini_get('safe_mode') && ! ini_get('open_basedir'))
		{
			...
			{
				$this->options[CURLOPT_FOLLOWLOCATION] = FALSE; //there is no requirement of redirect in our case //Maganiva
			}
		}
		...
		//	Maganiva
		// 	in case of redirect, curl automatically uses the previous options
		//	which is a post method and fires the request again.
		// 	In our case, we need to use only GET, hence the hack below

		if($this->info['http_code'] == 302){
			$this->option(CURLOPT_POST, FALSE);
			$this->option(CURLOPT_POSTFIELDS, null);
			...
			$redirectUrl = $retVal['Location'];
			
			$this->session = curl_init($redirectUrl);
					
			//works only with php 5.4 onwards
			//$this->session = curl_init($this->info['redirect_url']);
