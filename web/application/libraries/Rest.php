...
class REST
{
	...
	{
		//This is a patch by Maganiva - Shall remove it soon
		if(stristr($uri, '.csv')){
			$this->_ci->curl->option(CURLOPT_HEADER, TRUE);
		}
		return $this->_call('post', $uri, $params, $format,$is_array);
	}

	public function getByPost($uri, $params = array(), $format = NULL, $is_array = false)
	{
		return $this->post($uri,$params,$format,$is_array);
	}
	
	public function upload($uri, $params = array(), $format = NULL, $is_array = false)
	{
		return $this->_call('upload', $uri, $params, $format);
	}
	
	public function put($uri, $params = array(), $format = NULL, $is_array = false)
	{
		return $this->_call('put', $uri, $params, $format, $is_array);
	}
	...
	protected function _call($method, $uri, $params = array(), $format = NULL, $is_array = false, $apiType = 'prime',$retry_count=0)
	{
		...
		$this->http_header('Accept-Encoding', 'gzip, deflate');
		
		// Initialize cURL session
		// Handling of URL has been externalized from Rest, as the base URI might change from Server to Server.
		
		if(!strpos($uri,'authtoken=')){
			if(strpos($uri,'?')){
				$uri = $uri."&authtoken=" . $this->_ci->session->userdata('auth_token');
				//$this->_ci->curl->create($uri."&authtoken=" . $this->_ci->session->userdata('auth_token'));
			}else{
				$uri = $uri."?authtoken=" . $this->_ci->session->userdata('auth_token');
				//$this->_ci->curl->create($uri."?authtoken=" . $this->_ci->session->userdata('auth_token'));
			}
		...
		$this->_ci->curl->create($uri);
		
		$log_params = is_array($params)?json_encode($params):$params;
		log_message('debug', 'Rest request -'. $uri);
		log_message('debug', 'Rest Params -'. $log_params);
...
// 		echo $method . "<br>" . $uri . "<br>" . $params; die;
		
		// Execute and return the response from the REST server
		$response = $this->_ci->curl->execute();

		$header = $this->_ci->curl->info;
		if(!empty($header['http_code']) && $header['http_code'] == 401 ){
			throw new Unauthorized_Exception($params, "There is an issue with the server. Please try again later.", $header['http_code']);
		}
		
		if($response == null || $response == false || !isset($response)){
			throw new Communication_Exception($params, "There is an issue with the server. Please try again later.", $header['http_code']);
 		}
			
		//It might be possible, the response format and request format is different. Hence safe to fetch the format from the response content type
		
		if($this->_ci->curl->info['content_type'] != null){
			$this->format = $this->_ci->curl->info['content_type'];
		}
		// Format and return
		//log_message('debug', 'Rest Response Header -'. $header);
		log_message('debug', 'Rest Response -'. $response);
		
		$response =  $this->_format_response($response);
		//131
		if($response && isset($response->error_code)){
			if($response->error_code == 100 && $retry_count<2){
				log_message('debug', 'Retry Rest request count - '. $retry_count);
				$response = $this->_call($method, $org_uri, $params , $format, $is_array, $apiType, $retry_count+1);
			}else{
				throw new rest_exception($params,$response->error_message, $response->error_code);
			}
		}
	 ...
	 *
	 * Calls recursively to check and remove nulls from objects and converts the object into an array.
	 *
	 * Works for nested objects as well.
	 *
	 */
	protected function objectToArray($params){
		$array = array();
		foreach ($params as $member=>$data){
