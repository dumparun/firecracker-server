 ...
 *
 *	http://code.google.com/p/jquery-validation-php-plugin/wiki/Installation
 *
 * @package validator
 ...
 * @author Maganiva
 *
 *	Added a method to get all errors
 */
class Validator {
	...
	public $asOption =  array();
	protected $sErrorClass = 'error_message';
	protected $sErrorElement = 'div';
	 ...
	 * @author Maganiva
	 * 
	 * this method will give all error messages
	 */
	
	public function getAllErrorMessages(){
		$message = null;
		if (count ( $this->asErrorMessage ) > 0){
			foreach ($this->asErrorMessage as $value){
				if(is_array($value)){
					$message .= current($value) . " <br> ";
				}else{
				$message .= $value . " <br> ";
				}
			}
	...
	/**
	 * 
	 * @param array $asOption
	 */
	public function set_rules(array $asOption = null){
		...
		if (false === is_null($asOption)){
			...
			$_SESSION["validationrules"] = $asOption;
	}
	...
	public function showError($sField){
		...
		return '<'.
				$this->sErrorElement.
				($this->sErrorElement === 'label'? ' ':' html ').
				'for="'.$sField.'" generated="true" style="display:none;" class="'. 
				$this->sErrorClass.
				'">'. 
				'</'.$this->sErrorElement.'>';		
	}
	...
	//return the validator valid/invalid values
	public function getValidatorValue($sField){
		return	isset($this->asValue[$sField])? $this->asValue[$sField]:'';
	}
	...
	public function validate(array $asValue){
		$this->set_rules( $_SESSION["validationrules"] );
		$_SESSION["validationrules"] = "";
...
//				$this->asErrorMessage[$sField] = $this->getMessage($sField, 'required', $asRules['required']);
		...
		return empty($this->asErrorMessage)? TRUE : FALSE ;
	}
}
/**
...
class ValidatorMethodCollection extends ArrayObject {
  ...
  (
	...
	/* Hooked by Maganiva - $sSeperator made to be "/" compulsory - HACK */
	public function date($sValue, $sSeparator = '/'){
		$a = preg_match('~^[0-3][0-9]'."/".'[0-3][0-9]'."/".'[0-9]{4}$~', $sValue) === 1;
	}
