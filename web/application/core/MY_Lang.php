...
class MY_Lang extends CI_Lang
{
    ...
    public function load($langfile = '', $idiom = '', $return = FALSE, $add_suffix = TRUE, $alt_path = '',$cobrandkey = '')
    {
        $this->_set_language();
        $CI =& get_instance();
		$file = $langfile;
        ...
        if (in_array($cobrandkey.$langfile, $this->is_loaded, TRUE))
        {
            return;
        }
        ...
        $this->is_loaded[] = $cobrandkey.$langfile;
        // add prefix value of array key
        $lang = $this->_set_prefix($lang);
        $this->language = array_merge($this->language, $lang);
        unset($lang);
		
        //Load white label file if exist
        $white_label = strtolower($CI->input->get_post('white_label'));
        if(isset($white_label)){
	        if (file_exists(APPPATH.'cobrands/'. $white_label .'/language/'.$idiom.'/'.$langfile)){
	        	$this->load($file, $idiom, $return , $add_suffix , APPPATH.'cobrands/'. $white_label . '/',$white_label);
